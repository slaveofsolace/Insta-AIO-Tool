...
- Check both lists in the background from the matching open profile. Keep the visible list scan under **Capture lists and export**.
...
- Show verified follow-back results when Instagram supplies explicit relationship status, even if its lists disagree with the profile counters. Mark incomplete groups clearly; never infer a non-follower from a missing row.
- Keep these reviews separate from saved comparisons and action targets, with readable text and distinct JSON downloads.
- Hide the loading bar when the check finishes or stops.
...
- Avoid repeated six-second waits for next-page spinners below the visible list. Continue through loaded rows while still waiting for visible loading indicators.
