...
test('all public surfaces share the 3.1.11 Insta Toolbox identity', () => {
  assert.equal(packageMetadata.name, 'insta-toolbox');
  assert.equal(packageMetadata.version, '3.1.11');
  ...
  assert.equal(extensionManifest.version, '3.1.11');
  assert.equal(webManifest.name, 'Insta Toolbox');
  assert.match(userscriptMetadata, /@name\s+Insta Toolbox/);
  assert.match(userscriptMetadata, /@version\s+3\.1\.11\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v3111'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+3\.1\.11\s*$/m);
